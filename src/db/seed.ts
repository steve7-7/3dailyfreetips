...
import { users, predictions, subscriptions } from "./schema";
...
async function main() {
  ...
  const [premiumUser] = await db
    .select({ id: users.id })
    .from(users)
    .where(sql`${users.email} = 'premium@goaledge.com'`)
    .limit(1);
  ...
  if (predictionCount > 0) {
    console.log(`✓ Predictions already seeded (${predictionCount}). Skipping.`);
  } else {
    const rows = SEED_PREDICTIONS.map((p) => ({
      ...p,
      kickoffAt: p.kickoffAt,
    }));
    await db.insert(predictions).values(rows);
    console.log(`✓ Inserted ${rows.length} predictions.`);
  }
  ...
  if (existingSubs.count === 0 && premiumUser) {
    await db.insert(subscriptions).values([
      {
        userId: premiumUser.id,
        reference: "FP-SEED-MONTHLY-001",
        email: "premium@goaledge.com",
        amount: 5000,
        currency: "NGN",
        plan: "monthly",
